...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showWinnerHistoryBox) name:[BetGameModel getInstance].EVENT_SHOW_WINNER_HISTORY object:nil];
...
-(void)showWinnerHistoryBox
{
    //是否显示历史记录框
    if (_betGameModel.isShowWinnerHistory) {
        [_betGameMainView showHistoryBoxWithArr:_betGameModel.winnerHistoryArr];
    }else{
        [_betGameMainView hideHistoryBox];
    }
