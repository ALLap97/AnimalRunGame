...
-(void)countdownHandler
{
    ...
    if(_countDownIndex <=-1){
        //go
        [_countDownImgView setFrame:CGRectMake(self.bounds.size.width/2-53, self.bounds.size.height/2-10, 106, 55)];
        UIImage * picImg = [UIImage imageNamed:@"game_go_img.png"];
        [_countDownImgView setImage:picImg];
        [self performSelector:@selector(countDoneAndGameGo) withObject:nil afterDelay:0.3f];
    }else if(_countDownIndex <=0){
        // ready
        [_countDownImgView setFrame:CGRectMake(self.bounds.size.width/2-85, self.bounds.size.height/2-10, 193, 55)];
        UIImage * picImg = [UIImage imageNamed:@"game_ready_img.png"];
        [_countDownImgView setImage:picImg];
    }
    else
    {
...
-(void)finishRunAllAnimal
{
    ...
    //整个_animalSpr的UIview将会执行匀速平移的动画效果
...
-(void)stopAllAnimating
{
    ...
    //发送游戏结束，结算 事件  -> 显示所有动物最终位置并计算比赛结果
